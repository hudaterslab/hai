...
CONFIG_CAMERAS_FILE = os.path.join(PROJECT_ROOT, "config", "cameras.json")
...
def load_system_config():
    ...
    try:
        with open(CONFIG_COMMON_FILE, 'r', encoding='utf-8') as f:
            return deep_merge_dict(default_config, json.load(f))
    except Exception:
        return default_config

SYS_CFG = load_system_config()

def _log_nas_sync_worker(terminal_id, log_dir):
    while True:
        time.sleep(60)
        try:
            nas_root = os.path.expanduser("~/hai_nas/hudaters03")
...
def load_json_file(path, default, expected_type=None, return_meta=False):
    ...
    except Exception: return (copy.deepcopy(default), "invalid") if return_meta else copy.deepcopy(default)
...
def get_foot_point(x1, y1, x2, y2): return (int((x1 + x2) / 2), int(y1 + (y2 - y1) * (2/3)))
...
def send_event_image_to_receiver(image_path, event_name, terminal_id, cctv_id, bboxes, img_width=None, img_height=None):
    ...
    if event_name not in mapping: return
    ...
    try:
        ...
        logger.error(f"⚠️ [API 통신 실패]: {e}\n{traceback.format_exc()}")

def _upload_to_nas_task(fpath, file_type, ip, event_name, terminal_id):
    if not os.path.exists(fpath): return
    try:
        nas_root = os.path.expanduser("~/hai_nas/hudaters03")
        if not os.path.exists(nas_root):
            logger.error(f"⚠️ [NAS 저장 보류] 마운트가 끊겨 로컬 저장을 방지합니다.")
            return

        nas_base_dir = os.path.join(nas_root, str(terminal_id), file_type, event_name)
        os.makedirs(nas_base_dir, exist_ok=True)
        shutil.copy2(fpath, nas_base_dir)
        logger.info(f"☁️ [NAS 저장 완료] {os.path.basename(fpath)} -> {nas_base_dir}")
    except Exception as e:
        logger.error(f"⚠️ [NAS 저장 실패]: {e}")

def _save_and_send_task(img, img_path, api_params):
    try: 
        cv2.imwrite(img_path, img)
    except Exception as e: 
        logger.error(f"⚠️ [로컬 이미지 저장 실패]: {e}")
        return
        
    NAS_UPLOADER_POOL.submit(_upload_to_nas_task, img_path, "images", api_params['ip'], api_params['event_name'], api_params['terminal_id'])
...
def save_event_image_with_mark(frame, ip, event_type, bbox, tid, terminal_id="3", cctv_id=1):
    if IMAGE_SAVER_POOL._work_queue.qsize() > 50: return
    try:
        img = frame.copy()
...
class ConfigManager:
    ...
    def build_runtime_config(self):
        ...
        for ip, info in self.camera_configs.items():
            ...
            try: conf["cctv_id"] = int(conf.get("cctv_id", 1))
            except Exception: conf["cctv_id"] = 1
