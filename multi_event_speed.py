...
def load_system_config():
    default_config = {
        ...
        "event_config": {
            ...
            # 과속: px 이동량을 km/h로 바꾸려면 캘리브레이션된 카메라 모델이 반드시 필요합니다.
            # 값은 카메라마다 다르므로 cameras.json의 카메라별 event_config로 덮어씁니다.
            # camera_model이 없거나 값이 유효하지 않으면 속도를 알 수 없으므로 감지하지 않습니다.
            "speeding": {
                "enabled": False, "cooldown_sec": 10,
                "speed_threshold_kmh": 30.0,
                # 고정 카메라 핀홀 모델. 화면 전체의 차량 접지점을 지면 좌표로 변환한다.
                # image_width/height는 모델을 피팅한 원본 영상 해상도이며, 실행 프레임이
                # 축소되어도 자동으로 이 좌표계에 맞춰 환산한다.
                "camera_model": {
                    "focal_length_px": 0.0,
                    "camera_height_m": 0.0,
                    "tilt_deg": 0.0,
                ...
                # 원거리일수록 1px 오차가 큰 실제 거리로 증폭된다(지평선 근처는 1px = 수십 m).
                # 이 거리를 넘는 차량은 계측하지 않는다. ROI 게이팅을 대신하는 안전장치.
                "max_distance_m": 80.0,
                "measure_window_sec": 1.0,
                "min_samples": 3,
                "consecutive_hits": 3,
...
def _draw_event_api_image(
    frame, event_type, bbox, tid, objects_meta=None, auth_tokens=None, event_details=None
):
    ...
    for obj in draw_items:
        ...
        if event_type == "speeding" and is_target:
            try:
                speed_kmh = float((event_details or {}).get("speed_kmh"))
                label = f"speeding {speed_kmh:.1f} km/h"
    ...
    if event_type == "speeding":
        ...
        border_thickness = max(3, int(round(min(h_frame, w_frame) * 0.008)))
        speed_text = "speed unavailable"
        try:
            speed_kmh = float((event_details or {}).get("speed_kmh"))
        ...
        title = f"EVENT TRIGGERED: speeding | SPEED: {speed_text}"
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_scale = max(0.5, min(1.0, w_frame / 900.0))
        text_thickness = max(1, int(round(font_scale * 2)))
        (text_w, text_h), baseline = cv2.getTextSize(title, font, font_scale, text_thickness)
        if text_w > (w_frame - 24):
            font_scale *= (w_frame - 24) / float(text_w)
            (text_w, text_h), baseline = cv2.getTextSize(title, font, font_scale, text_thickness)

        banner_h = max(36, text_h + baseline + 16)
        overlay = api_img.copy()
        cv2.rectangle(overlay, (0, 0), (w_frame, banner_h), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.55, api_img, 0.45, 0, api_img)
        text_y = max(text_h + 6, (banner_h + text_h - baseline) // 2)
        cv2.putText(
            api_img, title, (12, text_y), font, font_scale,
            (0, 0, 255), text_thickness, cv2.LINE_AA
        )
        # Draw this last so the red frame remains vivid over the dark title banner.
        cv2.rectangle(
            api_img,
            (border_thickness // 2, border_thickness // 2),
            (w_frame - 1 - border_thickness // 2, h_frame - 1 - border_thickness // 2),
            (0, 0, 255),
            border_thickness
        )
...
def save_event_image_with_mark(frame, ip, event_type, bbox, tid, terminal_id="99999", cctv_id=1, objects_meta=None, trajectories=None, auth_tokens=None, event_id=None, event_ts=None, event_details=None):
    ...
    try:
        ...
        api_img = _draw_event_api_image(
            img, event_type, [x1, y1, x2, y2], tid,
            objects_meta, auth_tokens, event_details
        )
...
class VideoRecorder:
    def __init__(self, ip, cam_id=None):
        ...
        # Keep only the configured pre-event window, plus a small cadence margin.
        self.pre_sec = max(0.0, float(SYS_CFG.get("REC_PRE_SEC", 10.0)))
        buffer_fps = max(1.0, float(SYS_CFG.get("video_decode", {}).get("fps_limit", 15.0)))
        self.max_buffer_len = max(1, int(np.ceil(buffer_fps * (self.pre_sec + 1.0))))
    ...
    def update(self, frame, infer_meta=None, timestamp=None):
        ...
        if self.recording:
            # Use the frame timestamp so the clip stops at event + post seconds.
            if frame_timestamp > self.record_end_time:
                self.recording = False
    ...
    def trigger(self, event_name, objects_meta=None, event_meta=None, current_fps=3.0):
        now = time.time()
        post_sec = max(0.0, float(SYS_CFG.get("REC_POST_SEC", 5.0)))
        
        # [수정] 선에 처음 닿은 시점 역산 (Crossing Delay 반영)
        pre_sec = max(0.0, float(SYS_CFG.get("REC_PRE_SEC", 10.0)))
        # Anchor the clip to the event trigger; detector candidate age must not shift it.
        ...
        if self.recording:
            ...
            if isinstance(self.current_meta, dict):
                self.current_meta["recording_window_end_timestamp"] = self.record_end_time
        else:
            ...
            if isinstance(self.current_meta, dict):
                self.current_meta.update({
                    "event_trigger_timestamp": now,
                    "recording_window_start_timestamp": target_start_time,
                    "recording_window_end_timestamp": self.record_end_time,
                    "recording_pre_sec": pre_sec,
                    "recording_post_sec": post_sec
                })
    ...
    def _draw_speeding_video_overlay(self, frame, infer_meta):
        """Make every frame in a speeding clip visibly match the API evidence image."""
        if self.current_event != "speeding":
            return frame

        output = frame.copy()
        event_meta = self.current_meta if isinstance(self.current_meta, dict) else {}
        decision_trace = event_meta.get("decision_trace", {}) or {}

        speed_text = "speed unavailable"
        try:
            speed_text = f"{float(decision_trace.get('speed_kmh')):.1f} km/h"
        except (TypeError, ValueError):
            pass

        target_tid = None
        try:
            target_tid = int(event_meta.get("tid"))
        except (TypeError, ValueError):
            pass

        target_box = None
        if target_tid is not None and isinstance(infer_meta, dict):
            tracks = (infer_meta.get("tracks", {}) or {}).get("main", []) or []
            for track in tracks:
                try:
                    if int(track.get("tid")) == target_tid:
                        target_box = int_box(track.get("box", []))
                        break
                except (TypeError, ValueError):
                    continue
        ...
        if target_box is not None and len(target_box) >= 4:
            ...
            object_label = f"speeding {speed_text}"
            label_y = max(22, y1 - 8)
            cv2.putText(
                output, object_label, (x1, label_y), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        if text_w > (w_frame - 24):
            font_scale *= (w_frame - 24) / float(text_w)
            (_, text_h), baseline = cv2.getTextSize(title, font, font_scale, text_thickness)
        ...
        text_y = max(text_h + 6, (banner_h + text_h - baseline) // 2)
        cv2.putText(
            output, title, (12, text_y), font, font_scale,
            (0, 0, 255), text_thickness, cv2.LINE_AA
        )
    ...
    def _writer_loop(self):
        ...
        while self.running:
            ...
            if writer:
                ...
                writer.write(self._draw_speeding_video_overlay(frame, infer_meta))
                video_frame_index += 1
...
class SpeedingDetector(BaseEventDetector):
    """차량 과속 감지.

    프레임마다 차량의 접지점(bbox 하단 중앙)을 모아두고, 시간창(measure_window_sec) 양 끝
    표본의 이동거리를 실제 경과시간으로 나눠 평균 속도를 구합니다.
    FPS는 워커 스로틀·프레임 드롭으로 계속 흔들리므로 프레임 수가 아니라
    벽시계 시간(time.time())을 나눗셈의 분모로 씁니다.

    px->m 환산은 **캘리브레이션된 핀홀 카메라 모델만** 사용합니다. 초점거리·카메라 높이·
    틸트·롤로 차량 접지점을 지면 좌표로 역투영하므로 별도 ROI가 필요 없고,
    지평선 아래 화면 전체에서 동작합니다. 모델이 없으면 속도를 알 수 없으므로
    감지 자체를 하지 않습니다(엉터리 오탐 방지).

    단, 원거리는 1px 오차가 큰 실제 거리로 증폭되므로(지평선 근처는 1px = 수십 m)
    max_distance_m 을 넘는 차량은 계측하지 않습니다.
    """
    gui_name = "SPEEDING"
    ...
    def __init__(self, config, roi_poly=None, roi_lines=None):
        ...
        self.speed_threshold_kmh = float(config.get("speed_threshold_kmh", 30.0))
        self.measure_window_sec = max(0.2, float(config.get("measure_window_sec", 1.0)))
        self.min_samples = max(2, int(config.get("min_samples", 3)))
        self.consecutive_hits = max(1, int(config.get("consecutive_hits", 3)))
        self.max_plausible_kmh = float(config.get("max_plausible_kmh", 150.0))
        self.track_lost_sec = max(0.5, float(config.get("track_lost_sec", 2.0)))
        self.max_distance_m = float(config.get("max_distance_m", 80.0) or 0.0)

        camera_cfg = config.get("camera_model", {}) or {}
        self.camera_model = None
        try:
            model_f = float(camera_cfg.get("focal_length_px", 0.0) or 0.0)
            model_h = float(camera_cfg.get("camera_height_m", 0.0) or 0.0)
            model_tilt = math.radians(float(camera_cfg.get("tilt_deg", 0.0) or 0.0))
            model_roll = math.radians(float(camera_cfg.get("roll_deg", 0.0) or 0.0))
            model_w = int(camera_cfg.get("image_width_px", 0) or 0)
            model_img_h = int(camera_cfg.get("image_height_px", 0) or 0)
            if (model_f > 1.0 and model_h > 0.5 and 0.02 < model_tilt < 1.5
                    and model_w > 0 and model_img_h > 0):
                self.camera_model = {
                    "f": model_f,
                    "h": model_h,
                    "tilt": model_tilt,
        ...
        self.samples = defaultdict(lambda: deque(maxlen=120))  # tid -> deque[(ts, world_x_m, world_y_m)]
        self.hit_streak = defaultdict(int)
        self.peak_kmh = defaultdict(float)
        self.last_seen = {}
        self.last_speed_kmh = {}   # draw()에서 화면에 현재 속도를 띄우는 용도
        self.alerted_tids = set()
    ...
    def _to_world(self, pt, frame_shape=None):
        """화면 좌표(px) -> 지면 좌표(m). 캘리브레이션된 카메라 모델만 사용한다.

        지평선 위(전방 지면과 만나지 않는 광선)와 max_distance_m 초과 지점은 None을
        돌려 계측 대상에서 제외한다.
        """
        ...
        if frame_shape is not None:
            # 실행 프레임이 축소돼도 캘리브레이션 원본 해상도 좌표계로 환산한다.
            frame_h, frame_w = frame_shape[:2]
            if frame_w <= 0 or frame_h <= 0:
                return None
            u *= model["width"] / float(frame_w)
            v *= model["height"] / float(frame_h)

        du, dv = u - model["width"] / 2.0, v - model["height"] / 2.0
        cr, sr = math.cos(model["roll"]), math.sin(model["roll"])
        ct, st = math.cos(model["tilt"]), math.sin(model["tilt"])
        xc = du * cr + dv * sr
        yc = -du * sr + dv * cr
        d_down = model["f"] * st + yc * ct
        # 지평선 위의 광선은 전방 지면과 만나지 않는다.
        if d_down <= 1e-9:
            return None
        scale = model["h"] / d_down
        wx = xc * scale
        wy = (model["f"] * ct - yc * st) * scale
        if not (math.isfinite(wx) and math.isfinite(wy)):
            return None
        # 원거리는 1px 오차가 큰 실제 거리로 증폭돼(지평선 근처 1px = 수십 m)
        # 정지 차량도 과속으로 잡힐 수 있으므로 상한을 둔다.
        if self.max_distance_m > 0.0 and wy > self.max_distance_m:
            return None
        return (wx, wy)

    def _measure(self, tid, now_t):
        """시간창 양 끝 표본의 지면 이동거리 / 경과시간 = 평균 속도(km/h)."""
        buf = self.samples.get(tid)
        if not buf or len(buf) < self.min_samples:
            return None, 0.0, 0.0, 0

        window = [s for s in buf if (now_t - s[0]) <= self.measure_window_sec]
        ...
        t1, x1, y1 = window[-1]
        span = t1 - t0
        # 표본 수는 찼지만 시간창이 너무 짧으면(순간 폭주) 측정 오차가 커지므로 보류
        if span < (self.measure_window_sec * 0.5) or span <= 1e-6:
            return None, 0.0, 0.0, 0

        dist_m = math.hypot(x1 - x0, y1 - y0)
        return (dist_m / span) * 3.6, span, dist_m, len(window)

    def _forget(self, tid, drop_alerted=False):
        """트래킹이 끊기거나 표본을 믿을 수 없을 때 누적 상태를 버립니다."""
        self.samples.pop(tid, None)
        self.hit_streak.pop(tid, None)
        self.peak_kmh.pop(tid, None)
        if drop_alerted:
            self.last_seen.pop(tid, None)
            self.last_speed_kmh.pop(tid, None)
            self.alerted_tids.discard(tid)
    ...
    def process(self, tracks, track_map, motion_mask, frame, fid, **kwargs):
        ...
        if not self.is_calibrated:
            if not self._warned_uncalibrated:
                ...
                logger.warning(
                    "[Speeding] 캘리브레이션된 camera_model이 없어 과속 감지를 건너뜁니다. "
                    "cameras.json의 해당 카메라 event_config.speeding.camera_model에 "
                    "focal_length_px / camera_height_m / tilt_deg / roll_deg / "
                    "image_width_px / image_height_px 를 지정하십시오."
                )
            return triggered
        ...
        for t in tracks:
            ...
            x1, y1, x2, y2 = t[:4]
            ground_pt = get_check_point(x1, y1, x2, y2)  # 접지점(하단 중앙) = 지면과 닿는 지점

            # ROI는 사용하지 않는다. 지평선 아래이면서 max_distance_m 이내면 모두 계측한다.
            # (범위를 벗어나면 _to_world 가 None 을 돌려준다)
            world_pt = self._to_world(ground_pt, frame.shape[:2] if frame is not None else None)
            if world_pt is None:
                self._forget(tid)
                continue
            ...
            # [1차 방어] 프레임 간 '순간속도'로 트래커 ID 스위칭을 먼저 걸러냅니다.
            # 시간창 양 끝 평균만 보면 한 프레임짜리 점프가 창 길이만큼 희석돼
            # 그럴듯한 값(예: 52km/h)으로 남아 오탐이 됩니다.
            buf = self.samples[tid]
            if buf:
                prev_ts, prev_wx, prev_wy = buf[-1]
                step_sec = current_time - prev_ts
                if step_sec > 1e-6:
                    step_kmh = (math.hypot(world_pt[0] - prev_wx, world_pt[1] - prev_wy) / step_sec) * 3.6
                    if step_kmh > self.max_plausible_kmh:
                        logger.debug(
                            f"[Speeding] tid={tid} 프레임간 순간속도 {step_kmh:.1f}km/h "
                            f"(> {self.max_plausible_kmh}) -> ID 스위칭으로 보고 표본 폐기"
                        )
                        self._forget(tid)
                        buf = self.samples[tid]

            buf.append((current_time, world_pt[0], world_pt[1]))

            speed_kmh, span_sec, dist_m, n_used = self._measure(tid, current_time)
            if speed_kmh is None:
                continue

            # [2차 방어] 창 평균 자체가 비현실적이면(누적된 왜곡) 표본을 버리고 다시 모읍니다.
            if speed_kmh > self.max_plausible_kmh:
                logger.debug(
                    f"[Speeding] tid={tid} 비현실적 평균속도 {speed_kmh:.1f}km/h "
                    f"(> {self.max_plausible_kmh}) -> 표본 폐기"
                )
                self._forget(tid)
                continue

            self.last_speed_kmh[tid] = speed_kmh
            self.peak_kmh[tid] = max(self.peak_kmh[tid], speed_kmh)
            ...
            if self.hit_streak[tid] < self.consecutive_hits or tid in self.alerted_tids:
                continue

            self.alerted_tids.add(tid)
            triggered.append({
                'tid': tid,
                'bbox': t[:4],
                'frame': frame.copy() if frame is not None else None,
                ...
                'privacy_fid': fid,
                'decision_trace': {
                    'detector': 'SpeedingDetector',
                    'reason': 'speed_threshold_exceeded',
                    'class_id': int(track_map.get(tid, -1)),
                    'speed_kmh': round(float(speed_kmh), 2),
                    'peak_speed_kmh': round(float(self.peak_kmh[tid]), 2),
                    'speed_threshold_kmh': round(float(self.speed_threshold_kmh), 2),
                    'measured_distance_m': round(float(dist_m), 3),
                    'measured_span_sec': round(float(span_sec), 3),
                    'sample_count': int(n_used),
                    'consecutive_hits': int(self.hit_streak[tid]),
                    'consecutive_hits_required': int(self.consecutive_hits),
                    'calibration_mode': 'camera_model',
                    'distance_m': round(float(world_pt[1]), 2),
                    'max_distance_m': round(float(self.max_distance_m), 1),
                    'ground_point': int_point(ground_pt),
                }
            })

        # 사라진 트랙 정리(상태 dict가 무한히 커지지 않도록)
        for tid in list(self.last_seen.keys()):
            if tid in curr_ids:
                continue
            if (current_time - self.last_seen[tid]) > self.track_lost_sec:
                self._forget(tid, drop_alerted=True)
            ...
            for n in nums:
                ...
                if 0 <= local_idx < len(batch) and frames[local_idx] is not None:
                    ...
                    print(
                        ...
                        f"6.roi화각변경 7.roi화각변경+자동보정 8.과속"
                    )
                    ...
                    if '8' in selected_events: events.append("speeding")

                    roi_p = []
                    roi_l = []
                    cam_event_cfg = {}

                    # 과속 단독은 카메라 모델로 화면 전체를 측정하므로 ROI를 받지 않는다.
                    # 다른 ROI 기반 이벤트가 함께 있을 때만 그 이벤트용 ROI를 설정한다.
                    if any(e in events for e in ["intrusion", "illegal_parking", "no_helmet", "signal_vehicle"]):
                        poly_title = f"Polygon - CAM: {ip}"
                        roi_p = get_roi_points_scaled(frames[local_idx], poly_title)

                    if "speeding" in events:
                        print(f"[{ip}] 과속은 ROI 없이 지평선 아래 화면 전체 차량을 측정합니다.")
                        print("카메라 모델을 f(px), 높이(m), 틸트(deg), 롤(deg), 원본폭(px), 원본높이(px) 순서로 입력하십시오.")
                        print("PCTC 예: 2392.211914,7.894531,6.864258,0.166016,1920,1080")
                        raw_model = guarded_input(f"[{ip}] 카메라 모델 6개 값(빈 값=나중에 cameras.json에서 설정): ").strip()
                        if raw_model:
                            try:
                                vals = [float(v.strip()) for v in raw_model.split(',')]
                                if len(vals) != 6:
                                    raise ValueError("6개 값 필요")
                                f_px, cam_h_m, tilt_deg, roll_deg, img_w, img_h = vals
                                if f_px <= 1 or cam_h_m <= 0.5 or not (1.0 < tilt_deg < 85.0) or img_w <= 0 or img_h <= 0:
                                    raise ValueError("카메라 모델 값 범위 오류")
                                cam_event_cfg["speeding"] = {
                                    "camera_model": {
                                        "focal_length_px": f_px,
                                        "camera_height_m": cam_h_m,
                                        "tilt_deg": tilt_deg,
                                        "roll_deg": roll_deg,
                                        "image_width_px": int(round(img_w)),
                                        "image_height_px": int(round(img_h))
                                    }
                                }
                                logger.info(f"[{ip}] 전체화면 과속 카메라 모델 등록 완료")
                            except (TypeError, ValueError) as e:
                                logger.warning(
                                    f"[{ip}] 카메라 모델 입력 오류({e}). 과속 감지는 설정 전까지 비활성입니다. "
                                    f"cameras.json의 event_config.speeding.camera_model을 확인하십시오."
                                )
                        else:
                            logger.warning(
                                f"[{ip}] 카메라 모델이 입력되지 않았습니다. "
                                f"cameras.json의 event_config.speeding.camera_model을 직접 지정하십시오."
                            )
                    ...
                    if cam_event_cfg:
                        configs[ip]["event_config"] = cam_event_cfg
        except Exception as e:
...
class Camera:
    ...
    def _rebuild_handlers(self):
        ...
        global_event_cfg = SYS_CFG.get("event_config", {}) or {}
        # 카메라별 오버라이드: 과속의 px->m 환산처럼 값이 카메라마다 달라야 하는 항목을
        # cameras.json의 해당 카메라 event_config에서 개별 지정할 수 있게 합니다.
        camera_event_cfg = (self.conf.get("event_config", {}) or {}) if isinstance(self.conf, dict) else {}

        for ename in self.events:
            if ename in EVENT_REGISTRY:
                merged_cfg = deep_merge_dict(
                    global_event_cfg.get(ename, {}) or {},
                    camera_event_cfg.get(ename, {}) or {}
                )
    ...
    def run_logic(self, fr, fid, d_main_res, d_helmet_res, d_signalman_res=None):
        ...
        if len(self.fps_queue) > 1:
            ...
            self.current_fps = (len(self.fps_queue) - 1) / time_diff if time_diff > 0 else 0.0
        ...
        for ename, handler in self.handlers.items():
            ...
            for ev in triggered:
                ...
                # Use the fully merged per-camera event config (camera override > global default).
                cooldown = max(0.0, float(
                    getattr(handler, "config", {}).get(
                        "cooldown_sec",
                        SYS_CFG.get("event_config", {}).get(ename, {}).get("cooldown_sec", 600)
                    )
                ))
                ...
                if ename not in self.alerted[tid] and (now - self.last_evt_t.get(ename, 0) >= cooldown):
                    ...
                    self.recorder.trigger(
                        ...
                        current_fps=(
                            ...
                            else SYS_CFG.get("video_decode", {}).get("fps_limit", 15.0)
                        )
                    )
    ...
    def draw(self, fr, t_main, t_helmet, t_signalman, alarms, connected=True):
        ...
        if "illegal_parking" in self.events or "intrusion" in self.events or "speeding" in self.events:
        ...
        for t in t_main:
            ...
            # 과속 감지 중인 차량은 현재 측정 속도를 라벨에 함께 표시(캘리브레이션 검증용)
            if cls_id in TARGET_VEHICLES and "speeding" in self.handlers:
                measured_kmh = self.handlers["speeding"].last_speed_kmh.get(tid)
                if measured_kmh is not None:
                    label = f"{label} {measured_kmh:.0f}km/h"
...
def main():
    ...
    def run_camera_inference(cam, fr):
        ...
        if active_detection_events:
            ...
            # speeding은 모든 차량이 대상인데 V3(13클래스 통합 모델)에 승용차(3)와 트럭(6)이
            # 모두 들어있으므로 V3만으로 처리한다. V2를 추가로 돌릴 이유가 없다.
            v3_required = any(evt in active_detection_events for evt in ["signal_vehicle", "speeding"])
            ...
            # 승용차는 기본적으로 V2가 담당한다. 다만 V2를 아예 돌리지 않는 구성(예: speeding 단독)에서는
            # V3가 승용차까지 내야 하므로 그때만 V3 필터에 승용차를 추가한다(양쪽이 동시에 내면 박스가 중복됨).
            v3_classes = [ID_G_TRUCK, ID_SIGNALMAN, ID_RAINCOAT, ID_SIGNALFLAG]
            if "speeding" in active_detection_events and not v2_required:
                v3_classes.append(ID_G_CAR)

            filtered_v2 = [d for d in raw_dets_v2 if int(d[5]) in [ID_G_PERSON, ID_PERSON_LOW, ID_G_CAR]] if len(raw_dets_v2) > 0 else []
            filtered_v3 = [d for d in raw_dets_v3 if int(d[5]) in v3_classes] if len(raw_dets_v3) > 0 else []
