...
try:
    ...
    logger.warning(
        "[환경 알림] ultralytics를 찾을 수 없습니다. PT 모델 사용 불가."
    )
...
MODEL_SECTION_ALLOWED_KEYS = {
    ...
    "models_pt": frozenset(("MAIN", "FACE", "HELMET", "PLATE")),
...
def get_model_confidence(model_key):
    confidence_cfg = SYS_CFG.get("model_confidences")

    if not isinstance(confidence_cfg, dict):
        raise RuntimeError(
            "system_config.json에 'model_confidences' 설정이 없습니다."
        )

    key = str(model_key).upper()

    if key not in confidence_cfg:
        raise RuntimeError(
            f"system_config.json model_confidences에 "
            f"'{key}' 값이 없습니다."
        )

    try:
        value = float(confidence_cfg[key])
    except (TypeError, ValueError):
        raise RuntimeError(
            f"model_confidences.{key} 값이 숫자가 아닙니다: "
            f"{confidence_cfg[key]!r}"
        )

    if not 0.0 <= value <= 1.0:
        raise RuntimeError(
            f"model_confidences.{key} 값은 0~1이어야 합니다: {value}"
        )
...
class YoLoPyTorch:
    def __init__(
        ...
        if not HAS_ULTRALYTICS:
            raise RuntimeError(
                "PT backend requires ultralytics."
            )
        ...
        logger.info(
            f"[PyTorch][{self.model_key}] loading "
            f"{os.path.basename(model_path)} device={self.device}"
        )

        self.model = YOLO(model_path)

        logger.info(
            f"[PyTorch][{self.model_key}] model loaded"
        )
    ...
    def infer(self, img, conf_override=None):
        ...
        if conf_override is None:
            raise RuntimeError(
                f"[DeepX][{self.model_key}] "
                "confidence가 전달되지 않았습니다. "
                "system_config.json의 model_confidences를 확인하십시오."
            )

        threshold = float(conf_override)
        ...
        if result.boxes is not None:
            ...
            container_count = int(
                np.sum(classes == ID_PCTC_CONTAINER)
            )

            logger.warning(
                f"[PT RAW][{self.model_key}] "
                f"total={len(result.boxes)} "
                f"container={container_count} "
                f"conf={conf:.3f} "
                f"iou={self.iou:.2f} "
                f"imgsz={self.imgsz}"
            )
...
class YoLoDeepX:
    ...
    def infer(self, img, conf_override=None):
        ...
        if conf_override is None:
            raise RuntimeError(
                f"[DeepX][{self.model_key}] confidence가 전달되지 않았습니다. "
                "system_config.json의 model_confidences 설정을 확인하십시오."
            )
        ...
        try:
            ...
            raise ValueError(
                f"[DeepX][{self.model_key}] 잘못된 confidence 값: {conf_override!r}"
            )

        if not 0.0 <= threshold <= 1.0:
            raise ValueError(
                f"[DeepX][{self.model_key}] confidence는 0~1 범위여야 합니다: "
                f"{threshold}"
            )
        ...
        try:
            ...
            selected_format, reason = self._select_runtime_output_format(
                output_tensor
            )
            ...
            raw_detections = self._postprocess_by_format(
                selected_format,
                output_tensor,
                threshold
            )
            ...
            logger.error(
                ...
                f"({os.path.basename(self.engine_path)}): "
                f"{exc}\n{traceback.format_exc()}"
            )
            return np.empty((0, 6), dtype=float)
...
class ContainerCollisionRiskDetector(BaseEventDetector):
    ...
    def _find_carried_container(self, spreader, containers):
        ...
        for container in containers:
            ...
            if vertical_gap < -(container_height * 0.20):
                continue
...
class Camera:
    ...
    def apply_face_blur(self, frame, person_boxes, return_meta=False):
        ...
        blurred_faces = []
        try:
            face_conf = get_model_confidence("FACE")
            face_detections = self.det_face.infer(
                blur_img,
                conf_override=face_conf
            )
    ...
    def apply_plate_blur(self, frame, vehicle_boxes=None, return_meta=False):
        ...
        try:
            plate_conf = get_model_confidence("PLATE")
            p_dets = self.det_plate.infer(
                blur_img,
                conf_override=plate_conf
            )
...
def create_detector(
    ...
    if backend in ("auto", "dxnn"):

        if (
            HAS_DX_ENGINE
            and dxnn_path
            and os.path.exists(dxnn_path)
        ):
            try:
                logger.info(
                    f"[MODEL BACKEND] "
                ...
                return YoLoDeepX(
                    ...
                    output_format=
                        get_model_output_format(
                            model_key
                        ),

                    pool_size=
                        get_model_engine_pool_size(
                            model_key,
                            default=2
                            if model_key == "MAIN"
                            else 1
            ...
            message = (
                f"{model_key}: "
                "DXNN backend requested but "
                "dx_engine/model is unavailable."
            )
        ...
        if (
            ...
            try:
                ...
                return YoLoPyTorch(
                    ...
                    device=runtime_cfg.get(
                        "pt_device",
                        "auto"
                    ),

                    imgsz=runtime_cfg.get(
                        "pt_imgsz",
                        640
                    ),
    ...
    if required:
        raise RuntimeError(
            f"No usable backend for {model_key}"
        )

    logger.warning(
        f"[MODEL BACKEND] "
        f"{model_key} disabled - "
        f"no usable model"
    )
...
def main():
    ...
    models_pt_cfg = SYS_CFG.get("models_pt", {})
    event_inference_mode = "main"
    main_dxnn_path = resolve_model_path(models_cfg.get("MAIN", "pctc_v1.dxnn"))
    main_pt_path = resolve_model_path(models_pt_cfg.get("MAIN","pctc_v1.pt"))
    ...
    try:
        ...
        d_plate = create_detector(
            ...
            resolve_model_path(
                models_cfg.get(
                    "PLATE",
                    "license_plate_detector_v2.dxnn"
                )
            ),
    ...
    main_conf = get_model_confidence("MAIN")
    person_conf = get_model_confidence("PERSON")
    helmet_conf = get_model_confidence("HELMET")
    helmet_person_conf = get_model_confidence("HELMET_PERSON")
    face_conf = get_model_confidence("FACE")
    plate_conf = get_model_confidence("PLATE")
    ...
    try:
        ...
        person_merge_iou_threshold = 0.35
    logger.info(
        f"[PCTC RUNTIME] model={os.path.basename(main_dxnn_path)} "
        f"output_setting={get_main_model_output_format(main_dxnn_path)} "
    ...
    def run_camera_inference(cam, frame):
        ...
        raw_container_count = sum(
            1
            for d in raw_detections
            if int(d[5]) == ID_PCTC_CONTAINER
        )

        logger.warning(
            f"[BEFORE FILTER][MAIN] "
            f"total={len(raw_detections)} "
            f"container={raw_container_count} "
            f"main_conf={main_conf:.3f}"
        )
        ...
        logger.warning(
            f"[AFTER FILTER][MAIN] "
            f"total={len(d_main_res)} "
            f"container={kept_container_count} "
            f"main_conf={main_conf:.3f}"
        )
